...
n % 5 === 0 && console.log('5로 나누어 떨어질때만 실행') //값: 5로 나누어 떨어질때만 실행

// 앞 표현식의 평가 결과가 거짓일때는 뒤 표현식을 평가할 필요가 없어서 실행하지 않는다.

n = 6;

n % 5 === 0 && console.log('5로 나누어 떨어질때만 실행') //값: 
...
n % 5 === 0 || console.log('5로 나누어 떨어지지 않을때만 실행') //값: 

// 앞 표현식을 평가를 해서 거짓 일때만, 뒤 표현식을 평가할 필요가 생기기 때문에 뒤의 표현식이 실행된다.

n = 6;

n % 5 === 0 || console.log('5로 나누어 떨어지지 않을때만 실행') //값: 5로 나누어 떨어지지 않을때만 실행
